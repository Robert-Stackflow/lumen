...
static void wsi_pong(struct lws *wsi, struct pss_tty *pss) {
  if (pss->pong == NULL) return;
  char *message = xmalloc(LWS_PRE + 1 + pss->pong_len);
  char *ptr = message + LWS_PRE;

  *ptr = PONG;
  memcpy(ptr + 1, pss->pong, pss->pong_len);
  size_t n = pss->pong_len + 1;

  if (lws_write(wsi, (unsigned char *)ptr, n, LWS_WRITE_BINARY) < (int)n) {
    lwsl_err("write PONG to WS\n");
  }
    ...
    case LWS_CALLBACK_RECEIVE:
      ...
      switch (command) {
        ...
        case RESUME:
          ...
          break;
        case PING:
          if (pss->len <= 1 || pss->len > 65) break;
          if (pss->pong != NULL) free(pss->pong);
          pss->pong_len = pss->len - 1;
          pss->pong = xmalloc(pss->pong_len);
          memcpy(pss->pong, pss->buffer + 1, pss->pong_len);
          lws_callback_on_writable(wsi);
          break;
        case JSON_DATA:
          if (pss->process != NULL) break;
