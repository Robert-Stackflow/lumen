...
static const struct option options[] = {{"port", required_argument, NULL, 'p'},
                                        ...
                                        {"security-config", required_argument, NULL, 'E'},
                                        ...
                                        {NULL, 0, 0, 0}};
static const char *opt_string = "p:i:U:c:H:E:u:g:s:w:I:b:P:f:6aSC:K:A:Wt:T:Om:oqBd:vh";
...
static void print_help() {
  ...
  fprintf(stderr, "ttyd is a tool for sharing terminal over the web\n\n"
          ...
          "    -E, --security-config   Lumen security policy (session, proxy, or off mode)\n"
...
static void print_config() {
  ...
  if (server->auth != NULL) {
    const char *mode = server->auth->mode == LUMEN_AUTH_SESSION
                           ? "session"
                           : (server->auth->mode == LUMEN_AUTH_PROXY ? "proxy" : "off");
    lwsl_notice("  security policy: %s (%s)\n", mode, server->security_config);
  }
...
static void server_free(struct server *ts) {
  ...
  if (ts->auth != NULL) lumen_auth_free(ts->auth);
  if (ts->security_config != NULL) free(ts->security_config);
...
static int drop_privileges(int *uid, int *gid) {
#ifndef _WIN32
  if (geteuid() == 0 && (*uid >= 0 || *gid >= 0)) {
    if (*uid < 0 || *gid < 0) {
      fprintf(stderr, "ttyd: both --uid and --gid are required when starting as root\n");
      return -1;
    }
    if (setgroups(0, NULL) != 0 || setgid((gid_t)*gid) != 0 || setuid((uid_t)*uid) != 0) {
      fprintf(stderr, "ttyd: failed to drop privileges: %s\n", strerror(errno));
      return -1;
    }
    if (geteuid() != (uid_t)*uid || getegid() != (gid_t)*gid) {
      fprintf(stderr, "ttyd: privilege drop did not reach the requested uid/gid\n");
      return -1;
    }
    *uid = -1;
    *gid = -1;
  ...
  if (prctl(PR_SET_DUMPABLE, 0, 0, 0, 0) != 0) {
    fprintf(stderr, "ttyd: failed to disable process dumps: %s\n", strerror(errno));
    return -1;
  }
#endif
#else
  ...
  while ((c = getopt_long(start, argv, opt_string, options, NULL)) != -1) {
    switch (c) {
      ...
      case 'H':
        ...
        break;
      case 'E':
        server->security_config = strdup(optarg);
        break;
      case 'u':
...
#undef sc
  ...
  if (server->security_config != NULL) {
    if (server->credential != NULL || server->auth_header != NULL) {
      fprintf(stderr, "ttyd: --security-config cannot be combined with --credential or --auth-header\n");
      return -1;
    }
    char auth_error[256];
    if (lumen_auth_load(server->security_config, &server->auth, auth_error, sizeof(auth_error)) != 0) {
  ...
  // Read the root-owned security policy first, then permanently discard
  // supplementary groups and root before opening any listening socket.
  if (drop_privileges(&info.uid, &info.gid) != 0) return -1;
